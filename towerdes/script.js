...
/* ============================================================
   ...
   ============================================================ */
function updateMongCoOptions() {
  const loaiCot = document.getElementById("loaiCot").value;
  const soMongCoSelect = document.getElementById("soMongCo");

  if (loaiCot === "day_co") {
    // Cột dây co: chỉ cho 3, 4 móng
    const options = soMongCoSelect.querySelectorAll("option");
    options.forEach((opt) => {
      if (opt.value === "0") opt.style.display = "none";
      else if (opt.value === "3" || opt.value === "4")
        opt.style.display = "block";
      else opt.style.display = "none";
    });
    if (soMongCoSelect.value === "0") soMongCoSelect.value = "3";
  } else if (loaiCot === "tu_dung") {
    // Cột tự đứng: chỉ cho 0 móng
    const options = soMongCoSelect.querySelectorAll("option");
    options.forEach((opt) => {
      if (opt.value === "0") opt.style.display = "block";
      else opt.style.display = "none";
    });
    soMongCoSelect.value = "0";
  }
}
...
/* ============================================================
   GEOLOCATION: Lấy vị trí GPS hiện tại
   ============================================================ */
function getGPS(tabName) {
  if (!navigator.geolocation) {
    alert("❌ Điện thoại/trình duyệt không hỗ trợ GPS!");
    return;
  }
...
/* ============================================================
   MAP SELECTION: Bật/tắt chế độ chọn tọa độ trên bản đồ
   ============================================================ */
...
function veBanDoCot() {
  ...
  const loaiCot = document.getElementById("loaiCot").value;
  const huongCuaTram = parseFloat(
    document.getElementById("huongCuaTram").value,
  );
  ...
  const kichThuocHangRaoRong = parseFloat(
    document.getElementById("kichThuocHangRaoRong").value,
  );
  const kichThuocHangRaoDai = parseFloat(
    document.getElementById("kichThuocHangRaoDai").value,
  );

  // Validation: Kiểm tra tính hợp lệ
  if (loaiCot === "day_co" && soMongCo !== 3 && soMongCo !== 4) {
    alert("⚠️ Cột dây co chỉ cho phép 3 hoặc 4 móng co!");
    return;
  }
  if (loaiCot === "tu_dung" && soMongCo !== 0) {
    alert("⚠️ Cột tự đứng không có móng co (chọn 0)!");
    return;
  }
  ...
  if (loaiCot === "day_co" && soMongCo === 3) {
    ...
    [
      { x: half_l, y: half_w, a: 45 },
      { x: -half_l, y: half_w, a: 135 },
      { x: -half_l, y: -half_w, a: 225 },
      { x: half_l, y: -half_w, a: 315 },
    ].forEach((c) => {
